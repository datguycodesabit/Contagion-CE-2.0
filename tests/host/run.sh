...
case "$test_source" in test_rules|balance|test_ticker) ;; *) echo "Use test_rules, balance or test_ticker" >&2; exit 2;; esac
if [ "$#" -gt 0 ]; then shift; fi
${CC:-clang} -std=c99 -g -O1 -Wall -Wextra -Werror -pedantic \
  -fsanitize=address,undefined -fno-omit-frame-pointer \
  -Isrc "tests/host/$test_source.c" src/disease.c src/traits.c src/world.c src/savecodec.c src/ticker.c \
