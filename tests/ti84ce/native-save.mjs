...
function detail(id){
 key(1,0);const cat=traits[id].category;
 for(let i=0;i<cat;i++)key(1,3);
...
function spores(){key(1,0);key(1,3);key(1,3);key(1,2);}
...
detail(19);const blocked=d();key(1,7);assert.deepEqual(d(),blocked,'Cannot devolve a prerequisite');key(6,6);key(6,6);
detail(20);const before=d();key(1,7);assert.equal(d().dna,before.dna-7);assert(!(d().owned[0]&(1<<20)));shot('devolved');key(6,6);key(6,6);
assert.equal(stage(),2000);shot('complete');
